# Script: Visualizing METABRIC differential gene expression results from FPKM data
...
infile.4 <- "./data/METABRIC/2_transcriptomic/processed/DE_result_counts.RData"
infile.5 <- "./data/METABRIC/2_transcriptomic/processed/DE_result_FPKM.RData"
infile.6 <- "./data/METABRIC/2_transcriptomic/processed/Metagene_scores.RData"
# output paths
...
anno <- anno[anno$METABRIC_ID %in% unname(unlist(sample.ids)), c("METABRIC_ID","PAM50")]

# add metagene scores to the annotation
mg.scores <- as.data.frame(t(loadRData(infile.6)))
mg.scores$METABRIC_ID <- rownames(mg.scores)
rownames(mg.scores) <- NULL
anno <- merge(anno, mg.scores, by = "METABRIC_ID", all.x = TRUE)
...
#length(intersect(fpkm.degs.luma,fpkm.degs.lumb)) #396
...
# Basal_vs_LumA
anno.Basal_vs_LumA <- anno[anno$PAM50 %in% c("Basal","LumA"),]
dat.Basal_vs_LumA <- fpkm.dat[
  fpkm.degs.luma, colnames(fpkm.dat) %in% anno.Basal_vs_LumA$METABRIC_ID]
...
plot <- pheatmap(sampleDist,
         ...
         annotation_col = data.frame(Proliferation = anno.Basal_vs_LumA$Mitotic_progression,
                                     ...
                                     PAM50 = as.factor(anno.Basal_vs_LumA$PAM50),
                                     row.names = anno.Basal_vs_LumA$METABRIC_ID), 
...
# Basal_vs_LumB LumA
anno.Basal_vs_LumB <- anno[anno$PAM50 %in% c("Basal","LumB"),]
dat.Basal_vs_LumB <- fpkm.dat[
  fpkm.degs.lumb, colnames(fpkm.dat) %in% anno.Basal_vs_LumB$METABRIC_ID]
...
plot <- pheatmap(sampleDist,
                 ...
                 annotation_col = data.frame(Proliferation = anno.Basal_vs_LumB$Mitotic_progression,
                                             ...
                                             PAM50 = as.factor(anno.Basal_vs_LumB$PAM50),
                                             row.names = anno.Basal_vs_LumB$METABRIC_ID), 
                 annotation_colors = list(Proliferation = mg.colors,
                                          SteroidResponse = mg.colors,
                                          ImmuneResponse = mg.colors,
                                          PAM50 = color.palette),  
...
plot <- pheatmap(sampleDist,
                 ...
                 annotation_col = data.frame(Proliferation = anno$Mitotic_progression,
                                             ...
                                             PAM50 = as.factor(anno$PAM50),
                                             row.names = anno$METABRIC_ID), 
                 annotation_colors = list(Proliferation = mg.colors,
                                          SteroidResponse = mg.colors,
                                          ImmuneResponse = mg.colors,
                                          PAM50 = color.palette),  
