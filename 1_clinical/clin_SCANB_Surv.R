# Script: Survival analyses in the SCANB cohort based on DRFI , OS (SCANB rel4) 
...
txt.out <- append(txt.out,
                  c("log-rank test", "\n", 
                    capture.output(surv_pvalue(EC.fit, data = EC.dat)),"\n###########################################\n"))
...
txt.out <- append(txt.out,
                  c("log-rank test", "\n", 
                    capture.output(surv_pvalue(E.fit, data = E.dat)),"\n###########################################\n"))
