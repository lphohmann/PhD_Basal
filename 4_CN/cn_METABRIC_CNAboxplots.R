# Script: Plotting CNA boxplots in METABRIC
...
infile.6 <- "./data/METABRIC/4_CN/processed/CNA_GLFreqs_ERpHER2nAll.RData"
infile.7 <- "./data/METABRIC/4_CN/processed/CNA_genetest.RData"
infile.8 <- "./data/METABRIC/4_CN/processed/CNA_genelevel_all.RData"
# output paths
#outfile.1 <- ""
...
# name like the scnab file
colnames(gl.freqs) <- sub("ERpHER2n_", "", colnames(gl.freqs))
...
cna.df <- loadRData(infile.8)
row.names(cna.df) <- cna.df$gene
# only incl luminal comp samples
cna.df <- cna.df[,names(cna.df) %in% c(c("gene","chr","start","end"),unname(unlist(sample.ids)))]

# sample # %altered
basal.dat <- as.vector(apply(cna.df[sample.ids$ERpHER2n_Basal],2,function(x) {
  (sum(x!=0)/length(x))*100}))
luma.dat <-  as.vector(apply(cna.df[sample.ids$ERpHER2n_LumA],2,function(x) {
  (sum(x!=0)/length(x))*100}))
lumb.dat <-  as.vector(apply(cna.df[sample.ids$ERpHER2n_LumB],2,function(x) {
  (sum(x!=0)/length(x))*100}))
